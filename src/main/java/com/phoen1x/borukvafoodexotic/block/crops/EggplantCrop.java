...
public class EggplantCrop extends CropBlock implements FactoryBlock, TransparentPlant {
    ...
    public @Nullable ElementHolder createElementHolder(ServerWorld world, BlockPos pos, BlockState initialBlockState) {
        return new EggplantCrop.Model(initialBlockState);
    }
    public static class Model extends BlockModel {
        public static final ArrayList<ItemStack> MODELS = new ArrayList<>();
        static{
            for (int i = 0; i <= 5; i++){
                MODELS.add(BaseItemProvider.requestModel(Identifier.of(BorukvaFoodExotic.MOD_ID, "block/eggplant_crop_stage"+i)));
            }
        }
